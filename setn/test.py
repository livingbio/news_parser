...
test_num = args.num
test_categ = list(map(lambda x: int(x),cats[::2])) if type(cats[0])==str else cats
...
def main():
    ...
    print('Done testing parser-page, using %.4f seconds.'%(time.clock()-start_time))

    print('================testing get category urls=====================')
    start_time = time.clock()
    test_get_category_urls(test_categ ,test_num)
    print('Done testing get-category-urls, using %.4f seconds.'%(time.clock()-start_time))
    print("Test case generated time:",data[2],"\nDone testing.")
