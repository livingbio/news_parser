...
# ------------------------------------------------------------
    ...
    utility_string = 'https://graph.facebook.com/comments?id={}&filter=stream&fields=parent.fields(id),message,from,created_time,like_count{}'
    suffix = ''
    ...
    while True:  # loop until no more next page
        ...
        for datum in data:
            ...
            if 'parent' not in datum.keys():
                ...
                assert (parent_id in comments_dict.keys()
                        ), "parent not found !"
...
def get_category_urls(category_url, back_counting_offset=-1, straight_counting_offset=-1):
    ...
    for page in range(starting_page, last_page):
        ...
        container = soup.find(
            'div', {'id': 'categories_list', 'class': 'main_list_sty01'})
